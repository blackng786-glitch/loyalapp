...
- [x] Pricing DISPLAY (3-tier, no paywall yet — bank pending) — landing page 3-tier table (Free RM0 / Pro RM49 featured / Business RM99) replacing single "free forever" card; dashboard billing card 3-tier comparison + highlights current plan + "coming soon" note; /api/billing returns billingEnabled flag (false until STRIPE keys set, so upgrade button auto-activates when live). Verified landing visually (desktop 3-col + mobile stack) + dashboard card structure. NO feature gating added (current users keep full access) (2026-06-25)
...
- 💰 PRICING DECISION (2026-06-25): 3 tiers — Free RM0 / Pro RM49/mo / Business RM99/mo. Pro gates (when live): member tiers, auto campaigns, unlimited SMS, advanced analytics+CSV, up to 3 branches. Business adds: unlimited branches, white-label, multi-staff, priority support. Referral kept FREE (growth driver). Display built; feature gating + Stripe wiring DEFERRED until company bank account approved (Stripe needs it). When bank ready: set STRIPE_SECRET_KEY/STRIPE_PRICE_ID(s)/STRIPE_WEBHOOK_SECRET on Railway → billingEnabled flips true → then add the actual paywall enforcement + 'business' plan handling.
